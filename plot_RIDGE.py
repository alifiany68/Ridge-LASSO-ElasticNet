...
# Path data dan simpan gambar
data_path = '/storage/emulated/0/KULIAH/LAMPIRAN/data_standarisasi.xlsx'
save_path = '/storage/emulated/0/KULIAH/LAMPIRAN/cv_ridge_rmse.png'
...
plt.errorbar(np.log10(alphas), mean_rmse, yerr=std_rmse, fmt='-o', color='red', ecolor='gray', capsize=3, label='Ridge RMSE ± std')
plt.axvline(log_alpha_min, color='blue', linestyle='--', label=f'log(λ_min)={log_alpha_min:.2f}')
plt.xlabel('log10(λ)')
plt.ylabel('Root Mean Squared Error (RMSE)')
plt.title('Cross Validation parameter λ pada Ridge')
plt.legend()
plt.grid(True)
plt.tight_layout()
...
print(f"Plot Ridge disimpan di: {save_path}")
print(f"Ridge: alpha (λ) terbaik = {alpha_min:.6f}, RMSE = {rmse_min:.4f}")

# Setelah proses cross validation dan sebelum/atau sesudah plt.savefig(...)

print(f"Alpha (λ) terbaik: {alpha_min:.6f} (log10(λ)={log_alpha_min:.2f})")
